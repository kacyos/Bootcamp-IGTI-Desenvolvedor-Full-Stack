...
function calculateSalaryFrom(fullSalary) {
  ...
  const inssDiscountPercentage = (( discountINSS/ baseINSS) * 100);
  const irpfDiscountPercentage = (( discountIRPF/ baseINSS) * 100);
  const netSalaryPercentage = ((netSalary / baseINSS) * 100);
